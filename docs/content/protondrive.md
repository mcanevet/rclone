...
Proton Drive does not support updating modification times. The modification
time is stored inside the encrypted revision XAttr; updating it would require
creating a new revision, which is not currently implemented.
...
The directory cache is automatically invalidated by a background event poller
that calls the Proton Drive events endpoint every 30 seconds. This means that
changes made by other clients will be reflected in rclone within ~30 seconds.

The cache is currently optimised for the case where rclone is the primary
client. If there are multiple concurrent writers to the same path, there is a
small window where stale cached data may be served between poll intervals.
...
A background event poller invalidates the cache automatically approximately 
every 30 seconds so that changes made by other clients are picked up. If you 
are mounting ProtonDrive as a VFS with very strict consistency requirements, 
...
This backend is a pure-Go implementation of the Proton Drive API. It does not
depend on any third-party Proton API libraries. The API calls, SRP
authentication, and client-side end-to-end encryption (via
[gopenpgp/v2](https://github.com/ProtonMail/gopenpgp)) are all implemented
directly in the `backend/protondrive/protondriveapi` package.

There is no official public API documentation from Proton Drive. The
implementation is based on reading the open-sourced
[Proton Drive SDK](https://github.com/ProtonDriveApps/sdk) and observing
traffic from the official web client.

The following features are not currently supported:

- Setting modification times (returns `ErrorCantSetModTime`; mod time is stored
  inside the encrypted revision XAttr and updating it would require creating a
  new revision)
- Server-side copy (there is no server-side copy API; files must be
  downloaded and re-uploaded)
- Chunk upload (multipart / parallel upload of a single file)
